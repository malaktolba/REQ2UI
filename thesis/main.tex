...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item \textbf{Create Project} --- captures the project name and the natural-language description.
  ...
  \item \textbf{UI Code Preview} --- renders the generated HTML screens in an embedded preview frame with navigation between screens.
\end{itemize}
...
\subsection{UML and UI Code Generation}
Stage 9 emits three UML diagrams (use case, class, sequence) as Mermaid source, which the frontend renders client-side; the output is sanitised to strip code fences that would break rendering. Stage 10 is a two-pass process on Gemini: pass one produces a shared design system (navbar, footer, Tailwind theme), and pass two generates a complete HTML page for each functionally distinct screen. A categorisation heuristic (\texttt{selectDistinctScreens}) deduplicates near-identical screens (e.g.\ login vs.\ register) so generation effort is spent on genuinely different pages, capped at six. Gemini calls are funnelled through a throttled queue (minimum $\approx$4.5\,s spacing) to stay within free-tier rate limits, and ``thinking'' tokens are disabled so the output budget is spent on the visible HTML.
