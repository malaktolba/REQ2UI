...
A dependency-free \emph{metrics engine} scores a generated SRS against 32 measurable rubrics derived from IEEE~830/829, each expressed as a passed/total ratio and grouped into four dimensions --- \textbf{completeness} (were all artifacts and fields produced?), \textbf{conformance} (do they follow the required schema and phrasing, e.g.\ ``the system shall'' and \texttt{FR-NNN} identifiers?), \textbf{coverage} (is every requirement traced to a test case?), and \textbf{validity} (are priorities, NFR categories, and OWASP mappings drawn from the allowed sets?). The mean rubric score is reported as the SRS \emph{accuracy}. Two tools consume the engine: a database report (\texttt{npm run report}) that scores every completed project and derives per-stage timing from the recorded \texttt{pipeline\_stages} timestamps (no API cost), and a live benchmark (\texttt{npm run bench}) that re-generates a project while timing each stage with a monotonic clock and capturing per-call API latency. The results below were produced by the database report over the projects generated during development.

\subsection{SRS Quality Results}
Across the evaluated projects the mean SRS accuracy was \textbf{97.6\%}. Table~\ref{tab:accuracy} gives the breakdown by quality dimension. Conformance and validity are essentially perfect, confirming that the structured-prompting/JSON-schema approach reliably yields well-formed, standards-conformant artifacts. The small completeness shortfall is concentrated in two optional front-matter rubrics (project ``objectives'' and some narrative prose fields), which are occasionally omitted for terse inputs; the core requirement, test-case, traceability, UML, and UI-code artifacts score at or near 100\%.

\begin{table}[ht]
\centering
\begin{tabular}{l r}
\toprule
\textbf{Quality dimension} & \textbf{Mean score} \\
\midrule
Completeness & 93.9\% \\
Conformance & 100.0\% \\
Coverage & 97.4\% \\
Validity & 99.4\% \\
\midrule
\textbf{Overall accuracy} & \textbf{97.6\%} \\
\bottomrule
\end{tabular}
\caption{Generated-SRS accuracy by quality dimension (mean across evaluated projects).}
\label{tab:accuracy}
\end{table}

\subsection{Generation-Time Results}
Table~\ref{tab:gentime} reports the mean processing time of each pipeline stage. The cost is dominated by the language-model stages that emit the most content --- functional test cases ($\approx$47\,s) and UI code generation ($\approx$80\,s, which itself comprises a design-system call plus one call per screen) --- while the structured-extraction stages complete in single-digit seconds. The summed per-stage time is $\approx$173\,s; true end-to-end wall-clock time is lower because independent stages are executed in concurrent groups (Section~6.3.2), and is reported directly by \texttt{npm run bench}. This comfortably satisfies the response-time intent of NFR1 for inputs up to the 5000-character limit.
...
\midrule
1 & Requirement Extraction & 4.3 \\
2 & Functional Requirements & 5.3 \\
3 & Non-Functional Requirements & 1.3 \\
4 & Security Requirements & 26.4 \\
5 & Functional Test Cases & 46.5 \\
6 & Security Test Cases & 7.5 \\
7 & UI Wireframe Descriptions & 3.9 \\
8 & Traceability Matrix & 13.1 \\
9 & UML Diagrams & 4.6 \\
10 & UI Code Generation & 80.4 \\
\midrule
 & \textbf{Summed stage time} & \textbf{$\approx$173} \\
\bottomrule
\end{tabular}
\caption{Mean generation time per pipeline stage (database report).}
\label{tab:gentime}
\end{table}

\subsection{API Latency (NFR2)}
Per-call API latency is captured by an in-process collector that times every Groq and Gemini request --- success or failure --- and aggregates the samples into p50/p95/max per provider. The benchmark harness resets this collector before a run and emits the summary alongside the timing and accuracy tables, providing the direct measurement basis for NFR2 (external API calls within target latency, with a bounded 95th percentile). The retry-and-fallback logic (Section~6.3.1) also makes the collector a natural place to observe the failure rate that NFR8 addresses.
